...
omitted, with raw values placed in explicitly named payload-scoped fields that
cannot be confused with canonical bearings.
...
`quality.bearing_frame` (string, same vocabulary) and `quality.heading_source`
(string identifying the heading reference used for compensation, e.g.
`AHRS_TRUE`, `GPS_COURSE`, `FIXED_MOUNT_SURVEYED`). Because v1.0 is locked and
its `bearing` object rejects the `frame` key, this quality-scoped mechanism is
the only frame provenance available to v1.0-emitting producers.
