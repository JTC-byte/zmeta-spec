...
- *"Conformance claims' `release_hashes` are never cross-checked against the manifest."* — Refuted 3/3. The offered evidence is false: reconstructing all 20 commits shows the claims IN SYNC with the manifest at every single one, including `origin/main`'s tip. The claim files are in the manifest's hash-pinned `claims` group, so in-repo tampering fails the kernel gate loudly. The reproduction requires regenerating *without* `--update-claims`, which the only documented command includes, and a bare regeneration resets `release_id` and fails the pins. Making the check unconditional would break every honest third-party claim made against a prior release — which is why `--verify-contract-hash` is opt-in by design.
  *(Correction 2026-08-12: this refutation is falsified. Its two grounds
  decayed in opposite ways. "The only documented command includes
  `--update-claims`" stopped protecting at the v1.1.22 cut, which rebuilt
  the manifest without refreshing the claims. "The claim files are
  hash-pinned" never protected against this finding at all: a pin certifies
  the stale bytes' integrity, not their currency, so the drift shipped
  checksummed. Both example claims in every published v1.1.22 bundle carry
  two `release_hashes` entries stale against the manifest beside them. The
  in-sync observation across 20 commits was true when made; the
  refutation's error was concluding the class could not recur because it
  had not yet. Values and reproduction: `docs/release_claims_errata.md`.
  Tension logged as X2-01 in `docs/zmeta_doctrine_review_log.md`.)*
