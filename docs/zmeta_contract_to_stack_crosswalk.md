...
| Risk adjudication and operator-tunable policy | 3.3 | Policy use limits, validator details, gateway warning events, and event-side risk labels make soft acceptance filterable | `policy/timing-freshness.yaml`, `policy/lineage.yaml`, `policy/producer-authority.yaml`, `gateway/src/validators.py`, `gateway/src/gateway.py`, `conformance/must-pass.jsonl` | Enforced | Maintain risk-label and use-limit tests whenever adding tunable policy surfaces. | Maintain |
