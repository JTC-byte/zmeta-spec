...
| v1.1.0 expanded tasking remains bounded | 21.3 | v1.1.0 schema supports RETURN_TO_BASE, SCAN_RF, LAND, TAKEOFF with TTL/deconfliction boundaries | `schema/zmeta-event-1.1.0.schema.json`, `examples/zmeta-v1.1-examples.jsonl` | Enforced | Add more must-fail fixtures for each expanded task type before promotion. | P2 |
