...
| 1. Operating Model | Partially Enforced | Event sourcing, layer roles, projection boundaries, and the completeness-without-exhaustiveness doctrine are backed by schemas, policy, conformance classes, extension governance, bad-event fixtures, and adapter-harness evidence; append-only storage behavior remains deployment/gateway discipline. |
| 2. Version Semantics | Enforced | Canonical dispatcher and version-specific schemas isolate v1.0 and v1.1.0; the core semantic change threshold is governance-only and should be applied during future contract edits. |
| 3. Enforcement Model | Enforced | Schema, policy, gateway, encoding, conformance, and risk-adjudication surfaces exist. No single surface enforces all semantics. |
...
| 14. Operator State Projection and CoT/TAK Mapping | Enforced | CoT egress is STATE_EVENT-only; CoT/JREAP/MAVLink ingress state requires external promotion evidence before authoritative STATE_EVENT acceptance. |
...
| 20. Extension Registry and Namespace Governance | Enforced | Machine-readable extension registry, validator, and version-boundary tests reserve future names without making them valid vocabulary. |
...
| ZMeta is complete without becoming exhaustive mission ontology | 1.1 | Contract doctrine, extension registry boundaries, conformance class non-claimability, semantic bad-event corpus, representative adapter harness | `spec/semantics-contract.md`, `spec/extension-registry.yaml`, `spec/conformance-classes.md`, `conformance/bad-events/`, `conformance/adapter-harness/` | Partially Enforced | Maintain as review posture: mission-specific behavior belongs in policy packs, deployment config, adapters, profiles, extension branches, operator views, or mission plugins, not the core kernel. | Maintain |
| Core semantic changes require a concrete ambiguity, failure, safety/audit risk, or operational need not solvable outside core | 2.6 | Governance documentation, release hash discipline, worklog/audit record | `spec/semantics-contract.md`, `release/zmeta-release-manifest.yaml`, `docs/zmeta_refinement_worklog.md` | Partially Enforced | Apply as the default review gate before adding core semantics; use policy/profile/adapter/extension paths first when sufficient. | Maintain |
...
| Future concepts are not valid current vocabulary | 2.3, 3.3, 16-20 | Schema strictness rejects unknown core fields/subtypes; extension registry reserves future/proposed names; conformance classes make future/reserved/planned classes non-claimable | `schema/*.json`, `spec/semantics-contract.md`, `spec/extension-registry.yaml`, `spec/conformance-classes.md`, `conformance/conformance_classes.yaml`, `conformance/must-fail.jsonl` | Enforced | Maintain registry, conformance-class, and schema-leakage tests whenever future names are added. | Maintain |
...
| Observation/inference/fusion/state separation | 4.3, 7 | Schema and policy forbid identity/classification in observations, track IDs in inferences, raw features in state; shared adapter harness validates representative adapter outputs. | `schema/*.json`, `policy/semantics.yaml`, `gateway/src/validators.py`, `conformance/must-fail.jsonl`, `conformance/bad-events/must-fail.jsonl`, `conformance/adapter-harness/must-pass.jsonl` | Enforced | Maintain bad-event and adapter-harness coverage when adapters or layer rules change. | Maintain |
...
| UTC timestamp with trailing `Z` | 5.1, 5.2 | The trailing `Z` itself is schema-enforced on both branches, and offset or timezone-less forms fail on both; the governed must-fail corpus carries the UTC-offset negative. Beyond the `Z`, enforcement splits: the locked v1.0 `utcDateTime` pattern is `Z$` and nothing more, so malformed values such as `garbageZ` or year 0001 validate on that branch, while the v1.1.0 pattern enforces structural calendar shape. `format: date-time` is annotation-only in this stack, so `pattern` is the whole gate. The malformed-timestamp class is pinned by a gateway test outside the governed corpora, and cross-event plausibility is a warn-only gateway runtime check. Adapter helpers normalize native times to UTC-Z on ingress. | `schema/zmeta-event-1.0.schema.json`, `schema/zmeta-event-1.1.0.schema.json`, `conformance/must-fail.jsonl`, `gateway/tests/test_x1_01_ts_structural_shape.py`, `gateway/tests/test_ts_plausibility_window.py`, `adapters/ingress/time_utils.py`, `adapters/ingress/test_timestamp_normalization.py` | Partially Enforced | Maintain UTC-Z negative fixtures. Keep the v1.0 permissiveness disclosed on integrator-facing surfaces rather than narrowing the locked pattern; the split was adjudicated under doctrine X1-01 (closed 2026-08-03) because narrowing v1.0 would move its lock hash anchor. | Maintain |
...
| Risk adjudication and operator-tunable policy | 3.3 | Policy use limits, validator details, gateway warning events, event-side risk labels, and four rule classes make soft acceptance filterable without loosening locked semantics | `policy/timing-freshness.yaml`, `policy/lineage.yaml`, `policy/producer-authority.yaml`, `gateway/src/validators.py`, `gateway/src/gateway.py`, `conformance/must-pass.jsonl`, `conformance/conformance_classes.yaml` | Enforced | Maintain risk-label and use-limit tests whenever adding tunable policy surfaces; keep `FUTURE_EXTENSION` items non-claimable until versioned adoption. | Maintain |
...
| Replay labels and scenario bundle identity | 9.5 | Replay tools exist, but no replay event labels or scenario manifest schema | `tools/replay.py`, `examples/*.jsonl` | Future | Keep as technical sidecar test evidence first; add stable event references only after replay manifest design. | Future |
...
| CoT/JREAP/MAVLink ingress promotion | 4.5.1, 14 | Ingress templates attach promotion metadata; producer-authority policy rejects invalid promotion by default and labels warn/degrade/quarantine decisions; adapter harness checks representative promoted outputs. | `policy/producer-authority.yaml`, `adapters/ingress/cot/cot_to_zmeta_template.py`, `adapters/ingress/jreap/jreap_track_to_zmeta_template.py`, `adapters/ingress/mavlink/mavlink_to_zmeta_template.py`, `gateway/tests/test_external_state_promotion.py`, `conformance/adapter-harness/must-pass.jsonl` | Enforced | Maintain adapter tests and promotion fixtures; treat external reports as evidence unless active policy promotes them. | Maintain |
...
| SCHEMA_VIOLATION is diagnostic, not trust/quarantine state | 7.9, 16.4 | Policy reason codes, schema enums, conformance fixtures, and contract wording distinguish diagnostics from trust/quarantine state | `policy/semantics.yaml`, `policy/violation-codes.yaml`, `schema/*.json`, `gateway/src/gateway.py`, `spec/semantics-contract.md`, `conformance/must-pass.jsonl` | Enforced | Future branch should add POLICY_ADJUDICATION or equivalent clearer diagnostic subtype. | Future |
...
| Extension namespace safety | 20 | Schema strictness, README guidance, extension registry, and registry validator reserve undefined/future concepts without making them current vocabulary | `schema/README.md`, `schema/*.json`, `spec/semantics-contract.md`, `spec/extension-registry.yaml`, `tools/validate_extension_registry.py` | Enforced | Maintain registry/version-boundary tests whenever future names are added. | Maintain |
...
| Protobuf decoded JSON validation | 3.5, 23 | Protobuf decoder expands payload JSON; gateway validates decoded event; conversion CLI is projection-only | `spec/protobuf-encoding.md`, `schema/proto/zmeta_event_v1.proto`, `zmeta_proto.py`, `gateway/src/gateway.py`, `tools/convert_encoding.py` | Partially Enforced | Add explicit gateway/CLI negative tests for schema-invalid protobuf after decode. | P1 |
| Protobuf is not semantic authority | 3.5, 23 | Spec and proto comments say decoded events must pass JSON schema/policy | `spec/protobuf-encoding.md`, `schema/proto/zmeta_event_v1.proto`, `schema/README.md` | Enforced | Maintain until protobuf promotion decision. | Maintain |
...
| Expand adapter harness breadth | Adapter tests | Shared adapter conformance now covers representative outputs, but not every native-message variant for every adapter. | Additional fixtures for remaining native variants and future sensor-adapter class evidence. |
