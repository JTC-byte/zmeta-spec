...
# Category hashes the package metadata carries, in emitted order. Single
# source: tools/validate_release_package.py imports this list and compares
# every field in it, so a hash added here cannot ship unverified. The two
# lists were previously independent and had drifted - the builder wrote
# seven, the validator compared four, and policy_bundle_hash,
# extension_registry_hash and conformance_class_manifest_hash were
# unverified integrity claims in the file a consumer is most likely to read
# alone (R1-11 A-19).
...
def package_metadata(
    ...
    *,
    ...
    return {
        ...
        **{field: manifest[field] for field in METADATA_HASH_FIELDS},
